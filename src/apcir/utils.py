"""Shared utilities for apcir (consolidated from the former search/indexing utils.py + libs.py).

DRY refactor: search/ and indexing/{dense,splade}/ each carried a near-duplicate utils.py.
They are merged here. Per-symbol reconciliation (reviewed + approved):
  - union of symbols unique to either side;
  - byte-identical shared symbols kept once;
  - pstore: kept the indexing superset (extra `high_protocol`, default == old search behavior);
  - check_dir_exist_or_build: kept the indexing version (the search one had no callers);
  - load_collection (list, jsonl+tsv) and load_collection_dict (dict, tsv) BOTH kept (renamed);
  - set_seed: unified to the fuller deterministic version, parameterized by (seed, multi_gpu).
The generic indexing dataloaders StreamIndexDataset / CollateClass also live here.
"""
import os
import json
import pickle
...
def set_seed(seed, multi_gpu=False):
    # Unified (reviewed): superset of the former search/indexing versions. `seed` is a raw int.
    random.seed(seed)
    np.random.seed(seed)
